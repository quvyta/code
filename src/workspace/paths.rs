//! Where the workspace lives by default, and the platform the install commands are written for.
//!
...
/// QCode's name in its family, as the person reads it: the workspace is the folder of this name
...
/// The places of the machine the wizard offers a workspace from.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct HostDirs {
    /// The workspace QCode offers when the person has not chosen one, or `None` on a machine
...
impl HostDirs {
    ...
    #[must_use]
    pub fn detect() -> Self {
        Self { workspace: Family::QUVYTA.workspace_dir(APP_TITLE), documents: documents_dir() }
    }
}
...
mod tests {
    ...
    #[test]
    fn the_default_workspace_follows_the_name_the_desktop_gave_documents() {
        const PRINT: &str = "QCODE_TEST_PRINT_WORKSPACE";
        if std::env::var_os(PRINT).is_some() {
            println!(
                "workspace={}",
                HostDirs::detect().workspace.map(|path| path.display().to_string()).unwrap_or_default()
            );
            return;
        }
        ...
        let printed = crate::testing::in_child(
            "workspace::paths::tests::the_default_workspace_follows_the_name_the_desktop_gave_documents",
            &[(PRINT, "1".as_ref()), ("HOME", home.as_os_str())],
        );

        let expected = home.join("Belgeler").join("Quvyta").join("Code");
        assert!(printed.contains(&format!("workspace={}\n", expected.display())), "{printed}");
        let _ = std::fs::remove_dir_all(&home);
    }
}
