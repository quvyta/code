//! The identifier a project is known by on disk, in container names and in the config.
//!
...
/// The name a project has on disk: lower-case ASCII letters and digits, separated by `-`.
///
...
/// legal container name and a legal volume name, so the same string names the project
/// everywhere.
...
impl ProjectId {
    ...
    ///
    /// The identifier is part of `qcode-home-<project>-<profile>`, of a folder name below the
    /// workspace and of a volume name; 64 leaves room for all of them inside the 255 characters
    /// the strictest of them allows.
    pub const MAX_LEN: usize = 64;

    /// Builds the identifier of a project the user named `name`.
    ///
    ...
    /// # Errors
    ///
    /// [`ProjectIdError::Empty`] when nothing in `name` can be part of an identifier.
    pub fn from_display_name(name: &str) -> Result<Self, ProjectIdError> {
    ...
    /// `id` of a `project.qcode`.
    ///
    /// # Errors
    ///
    /// Says which rule the text breaks, so the caller can point at it.
    pub fn parse(text: &str) -> Result<Self, ProjectIdError> {
        let length = text.chars().count();
        if length == 0 {
            return Err(ProjectIdError::Empty);
        }
        if length > Self::MAX_LEN {
            return Err(ProjectIdError::TooLong { length });
        }
        for (position, character) in text.chars().enumerate() {
            ...
            if !legal {
                return Err(ProjectIdError::Illegal { position, character });
            }
        }
        if RESERVED.contains(&text) {
            return Err(ProjectIdError::Reserved);
        }
    ...
    /// Whether a file system that ignores case would see `name` as this project's folder.
    ///
...
mod tests {
    ...
    fn id(name: &str) -> String {
        ProjectId::from_display_name(name).expect("a usable name").as_str().to_owned()
    }
    ...
    fn spaces_dots_and_case_become_one_separator() {
        assert_eq!(id("  My.Project   v2  "), "my-project-v2");
    ...
    fn windows_device_names_are_moved_out_of_the_way() {
        for reserved in ["con", "CON", "Prn", "aux", "NUL", "com1", "COM9", "lpt1", "LPT9"] {
            ...
            assert!(ProjectId::parse(&made).is_ok(), "{made} is a legal id");
        }
    ...
    fn a_name_with_nothing_usable_in_it_is_an_error() {
        for empty in ["", "   ", "...", "///", "。。"] {
            assert_eq!(ProjectId::from_display_name(empty), Err(ProjectIdError::Empty), "{empty:?}");
        }
    }

    #[test]
    fn a_long_name_is_cut_without_leaving_a_dangling_separator() {
        let made = id(&format!("{} {}", "a".repeat(ProjectId::MAX_LEN - 1), "bbbb"));
        assert_eq!(made.len(), ProjectId::MAX_LEN - 1, "cut at the separator, which is then trimmed");
        let long = id(&"ş".repeat(ProjectId::MAX_LEN * 2));
        assert_eq!(long.len(), ProjectId::MAX_LEN);
        assert!(ProjectId::parse(&long).is_ok());
    }

    #[test]
    fn parsing_accepts_only_what_generating_produces() {
        assert_eq!(ProjectId::parse("my-project").map(|id| id.as_str().to_owned()), Ok("my-project".to_owned()));
        assert_eq!(ProjectId::parse(""), Err(ProjectIdError::Empty));
        assert_eq!(ProjectId::parse("My-Project"), Err(ProjectIdError::Illegal { position: 0, character: 'M' }));
        assert_eq!(ProjectId::parse("a b"), Err(ProjectIdError::Illegal { position: 1, character: ' ' }));
        assert_eq!(ProjectId::parse("-a"), Err(ProjectIdError::Illegal { position: 0, character: '-' }));
        assert_eq!(ProjectId::parse("a-"), Err(ProjectIdError::Illegal { position: 1, character: '-' }));
        assert_eq!(ProjectId::parse("con"), Err(ProjectIdError::Reserved));
        let long = "a".repeat(ProjectId::MAX_LEN + 1);
        assert_eq!(ProjectId::parse(&long), Err(ProjectIdError::TooLong { length: ProjectId::MAX_LEN + 1 }));
    }

    #[test]
    fn a_clash_ignores_case_because_some_file_systems_do() {
        let made = ProjectId::from_display_name("Belgeler").expect("usable");
        ...
        assert!(!ProjectId::from_display_name("ilik").expect("usable").clashes_with("ILİK"));
    }
}
