...
//!
//! git is always told the same things: the backup's own git folder, the project as the tree it
...
/// line added to the project's own `.gitignore` — is taken out of the backup's index first, or
...
/// Brings `$2` back as it was in the snapshot `$1`, `.` for the whole project.
///
...
mod tests {
    ...
    fn paths() -> ProjectPaths {
        let root = PathBuf::from("/home/me/QCode/Projects/p");
        ProjectPaths {
    ...
    fn container(project: &str, script: &str) -> Vec<String> {
        [
            "run",
            "--rm",
            "--userns=keep-id",
            "--network=none",
            "--volume",
            &format!("/home/me/QCode/Projects/p/Project:/work/Project:{project},z"),
            "--volume",
    ...
    #[test]
    fn a_restore_is_the_one_job_that_writes_to_the_project() {
