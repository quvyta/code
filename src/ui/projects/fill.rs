//! Filling a project that has just been made: copying a folder into it, or cloning a git
...
//! project folder it was filling, so the list never grows a project that is not a project.
//!
...
use crate::base::{self, paths::KEEP_ALIVE, paths::PROJECT_DIR};
...
/// How a new project's `Project/` folder is filled.
#[derive(Debug)]
pub enum Fill {
...
pub struct Job {
    /// What to do.
    pub fill: Fill,
    /// The project's own folder, removed whole when anything goes wrong.
    pub root: PathBuf,
    /// The folder the content lands in.
    pub into: PathBuf,
    /// The project's identifier, which names the container the clone borrows.
    pub id: String,
}
...
pub fn run(job: &Job, cx: &TaskCx<Msg>) -> Result<Msg, String> {
    ...
    // Cancelling is not an error and still leaves a project that was never filled, so the
...
/// machine without one.
struct ClonePlan {
    /// The container's name, which is the project's own base container.
    container: String,
...
impl ClonePlan {
    /// Plans a clone of `url` into `into`, in the base container of the project `id`.
    fn new(engine: &Engine, url: &str, into: &Path, id: &str, user: HostUser) -> Self {
        let container = names::base_container(id);
        // The project folder lands where every later container will see it, so the clone is
        // already in the place the contract names rather than moved there afterwards.
        let workdir = Path::new(PROJECT_DIR);
...
///
/// The container is the project's own base container: nothing new has to be named for this, and
/// it is removed again afterwards so the project screen creates it later with the mounts it
/// really wants. The project folder is mounted read-write and is the working folder, so the
...
mod tests {
    ...
    #[test]
    fn a_clone_lands_where_every_later_container_looks_for_the_project() {
        ...
        assert!(create.iter().any(|word| word == &format!("/w/Project:{PROJECT_DIR}:rw")), "{create:?}");
        assert!(create.windows(2).any(|pair| pair[0] == "--workdir" && pair[1] == PROJECT_DIR), "{create:?}");
