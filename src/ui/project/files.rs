...
impl FileTree {
    ...
    pub fn new(root: impl Into<PathBuf>) -> Self {
        // The project folder is the tree's top row, and it starts open: its entries are what the
        // tree is for.
    ...
    /// wherever the folder goes; the project folder itself is never taken.
    #[must_use]
    ...
    /// open too, the project folder first when it is. A folder left open inside a closed one is
...
/// `keys` without any key inside a folder that is also among them, and without the project
...
pub enum FileMsg {
    ...
    DropCut,
    /// Entries were dragged onto a folder, or onto the free space that stands for the project
    /// folder.
    Drop(TreeDrop),
    ...
    CloseNaming,
    /// Operations on a project's files finished or were refused: the project's id, and for each
    /// entry its key and what came of it.
    Done(String, Vec<(String, Result<Change, FileError>)>),
}

/// Applies a message of the file tree to the open project.
pub(super) fn update(screen: &mut ProjectScreen, message: FileMsg) -> Command<Msg> {
    if let FileMsg::Done(id, results) = message {
        let Some(project) = screen.project_mut(&id) else { return Command::none() };
        return done(project, results);
    }
    let Some(project) = screen.projects.get_mut(screen.active) else { return Command::none() };
    match message {
        FileMsg::NewFile(folder) => ask_name(project, NameFor::File, folder, String::new()),
        FileMsg::NewFolder(folder) => ask_name(project, NameFor::Folder, folder, String::new()),
        FileMsg::Rename(key) => {
            let (folder, name) = (parent_key(&key).to_owned(), name_of(&key).to_owned());
            ask_name(project, NameFor::Rename(key), folder, name)
        }
        FileMsg::Choose(keys) => {
            project.files.choose(keys);
            Command::none()
        }
        FileMsg::Cut(key) => {
            project.files.cut = project.files.targets(&key);
            Command::none()
        }
        FileMsg::DropCut => {
            project.files.cut.clear();
            Command::none()
        }
        FileMsg::Paste(into) => {
            let keys = project.files.cut.clone();
            move_all(project, keys, into)
        }
        FileMsg::Drop(TreeDrop { keys, into }) => move_all(project, outermost(keys), into.unwrap_or_default()),
        FileMsg::Delete(key) => ask_delete(&project.files, project.files.targets(&key)),
        ...
        FileMsg::Submit => submit(project),
        FileMsg::CloseNaming => {
            project.files.naming = None;
            Command::none()
        }
...
fn ask_name(project: &mut OpenProject, purpose: NameFor, folder: String, value: String) -> Command<Msg> {
    let read = folder != ROOT && project.files.expand(&folder, true);
    let command = if read { super::read_folder(project, &folder) } else { Command::none() };
    project.files.naming = Some(Naming { purpose, folder, value, tried: false });
    command
}

/// Takes the name in the dialog, or points out what is wrong with it.
fn submit(project: &mut OpenProject) -> Command<Msg> {
    let Some(naming) = project.files.naming.as_mut() else { return Command::none() };
    naming.tried = true;
    if project.files.naming_problem().is_some() {
        return Command::none();
    }
    let Some(Naming { purpose, folder, value: name, .. }) = project.files.naming.take() else { return Command::none() };
    match purpose {
        NameFor::File => run_each(project, vec![folder], move |root, folder| {
            (child_key(folder, &name), file_ops::create_file(root, folder, &name))
        }),
        NameFor::Folder => run_each(project, vec![folder], move |root, folder| {
        ...
        NameFor::Rename(key) => {
            run_each(project, vec![key], move |root, key| (key.to_owned(), file_ops::rename(root, key, &name)))
        }
    }
}
...
fn move_all(project: &OpenProject, keys: Vec<String>, into: String) -> Command<Msg> {
    if keys.is_empty() {
        return Command::none();
    }
    run_each(project, keys, move |root, key| (key.to_owned(), file_ops::move_into(root, key, &into)))
}
...
fn ask_delete(files: &FileTree, keys: Vec<String>) -> Command<Msg> {
    ...
    let (title, message) = match keys.as_slice() {
        ...
        [key] => {
            let name = name_of(key);
            let text = if folders { "project.files.delete-folder-text" } else { "project.files.delete-file-text" };
            (t!("project.files.delete-title", name = name), t!(text, name = name))
        }
        many => {
            let mut names = many.iter().take(NAMES_SHOWN).map(|key| name_of(key)).collect::<Vec<_>>().join(", ");
            if many.len() > NAMES_SHOWN {
                names = format!("{names} {}", t!("project.files.and-more", n = many.len() - NAMES_SHOWN));
            }
            let text =
                if folders { "project.files.delete-many-folders-text" } else { "project.files.delete-many-text" };
            (t!("project.files.delete-many-title", n = many.len()), t!(text, names = names.as_str()))
        }
    };
    let label =
        if keys.len() > 1 { t!("project.files.delete-many", n = keys.len()) } else { t!("project.files.delete") };
    Command::confirm(
...
/// project folder, one after the other: a move of several entries is done in the order they
...
fn done(project: &mut OpenProject, results: Vec<(String, Result<Change, FileError>)>) -> Command<Msg> {
    ...
    touched.sort();
    touched.dedup();
    Command::batch([refusal(total, &refused), reread(project, touched)])
}
...
fn refusal(total: usize, refused: &[(String, FileError)]) -> Command<Msg> {
    match refused {
        [] => Command::none(),
        [(_, error)] if total == 1 => Command::toast(Toast::danger(t!("project.files.failed")).body(error.message())),
        _ => {
            let lines: Vec<String> =
                refused.iter().map(|(key, error)| format!("{}: {}", name_of(key), error.message())).collect();
            let title = t!("project.files.failed-some", failed = refused.len(), total = total);
...
pub(super) fn reread(project: &mut OpenProject, keys: Vec<String>) -> Command<Msg> {
    let keys: Vec<String> = keys.into_iter().filter(|key| key == ROOT || project.files.is_open(key)).collect();
    let commands: Vec<Command<Msg>> = keys
        .into_iter()
        .map(|key| {
            project.files.loading.insert(key.clone());
            super::read_folder(project, &key)
        })
        .collect();
...
pub(super) fn refresh(project: &mut OpenProject) -> Command<Msg> {
    let shown = project.files.shown_folders();
    reread(project, shown)
}
...
/// come back from the session file, which anyone can edit. A key that climbs out of the project
...
/// Why a document of the project could not be read.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum DocumentTrouble {
    /// There is no file there any more.
    Missing,
    /// The file is a link that leads out of the project folder.
    Outside,
...
/// in the project that points somewhere else on this machine. Text that is not UTF-8 is shown
