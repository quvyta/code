//! What the page of a blank tab knows about the conversations each profile had in a project:
...
/// The conversations of one profile in one project, as far as the page knows them.
...
pub(super) enum Shown<'a> {
    ...
    Failed(&'a str),
    /// The conversations, newest first; empty when the profile has had none in the project.
    Conversations(&'a [Conversation]),
}

/// One profile's conversations in one project, with the reading that keeps them fresh.
...
pub(super) fn when(used_ms: i64, now: DateTime) -> String {
    ...
    match days {
        0 => t!("project.history.today", time = time),
        1 => t!("project.history.yesterday", time = time),
