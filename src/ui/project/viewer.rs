//! The tabs that show a file's text taken out in the project's own container: a PDF, with the
...
use super::{DOCUMENT_ID, Msg, Pages, ProjectScreen, Tab, TabKey, TabKind, TabState, inside_container};
...
/// Takes the text of the tab `key`'s file out in the project's own container, on a background
/// thread: the file is looked for on this machine first, as for every file tab, then the
/// container is brought up and [`ProjectScreen::read_command`] run in it.
pub(super) fn take_text(screen: &ProjectScreen, key: TabKey, run: u64) -> Command<Msg> {
    let Some(project) = screen.projects.iter().find(|project| project.tabs.iter().any(|tab| tab.key() == key)) else {
        return Command::none();
    };
    let Some(kind) = project.tabs.iter().find(|tab| tab.key() == key).map(Tab::kind) else {
        return Command::none();
    };
    let (Some(engine), Some(plan), Some(command), Some(file)) =
        (screen.engine.clone(), project.plan(kind), screen.read_command(key), kind.file())
    else {
        return Command::none();
    };
    let file = project.files.path(file);
...
/// the tab kept.
pub(super) fn show_page(screen: &mut ProjectScreen, key: TabKey, page: Option<usize>) -> Command<Msg> {
...
fn pdf(tab: &Tab, file: &str, ui: &mut View<'_, Msg>) {
    ...
    if pages.drawn {
        ...
        ui.column(|ui| {
            ui.row(|ui| {
                ...
                if has_text {
                    ui.add(Button::new(t!("project.pdf.text")).on_press(Msg::Page(key, None)))
                        .id("project-text-action");
                }
            })
            .gap(2)
            ...
            if failed {
                ui.row(|ui| {
                    ui.add(Text::new(t!("project.pdf.not-drawn")).role("secondary"));
                })
                .padding(Padding { left: 2, right: 1, ..Padding::default() })
                .fill_width();
            }
            ui.row(|ui| {
                let previous = Button::new(t!("project.pdf.previous"))
                    .icon("arrow-left")
                    .disabled(pages.page <= 1)
                    .on_press(Msg::Page(key, Some(pages.page.saturating_sub(1))));
                ui.add(previous).id("project-page-previous");
                ui.add(Text::new(t!("project.pdf.page", page = pages.page, count = pages.count)).no_wrap());
                let next = Button::new(t!("project.pdf.next"))
                    .icon("arrow-right")
                    .disabled(pages.page >= pages.count)
                    .on_press(Msg::Page(key, Some(pages.page + 1)));
                ui.add(next).id("project-page-next");
                ui.spacer();
    ...
    let draw = Button::new(t!("project.pdf.picture")).on_press(Msg::Page(key, Some(pages.page)));
    ...
    ui.column(|ui| {
        ui.row(|ui| {
            ...
            if let Some(action) = action {
                ui.add(action).id("project-text-action");
            }
        })
        .gap(2)
        .padding(Padding { left: 2, right: 1, ..Padding::default() })
        ...
        let cut = tab.is_partial().then(|| t!("project.file.cut"));
        ...
        ui.add_with(ScrollView::new(), |ui| {
            ui.column(|ui| {
                if apps::is_blank(text) {
                    ui.add(Text::new(t!("project.file.no-text")).role("faint"));
                } else {
