//! The files whose text is taken out in the project's container and shown by QCode: a PDF, with
...
use crate::ui::project::{LaunchFailure, Pages, Taken, entry};
use crate::workspace::{Session, SessionProject, SessionTab, SessionTabKind};
...
/// A project folder with a PDF in it.
fn with_pdf(name: &str) -> Scratch {
    let scratch = Scratch::new(name);
    let papers = scratch.paths().project.join("papers");
...
/// The PDF opened in the open project, and the key of its tab.
fn open_pdf(screen: &mut ProjectScreen) -> TabKey {
...
#[test]
fn a_pdf_opens_as_its_text_taken_out_in_the_projects_own_container() {
    ...
    assert_eq!(
        read,
        ["exec", "qcode-firefly-base", "pdftotext", "-layout", &format!("{PROJECT_DIR}/{PDF}"), "-"],
...
fn the_text_is_shown_by_qcode_with_the_way_to_draw_a_page() {
    ...
    assert_eq!(entry(&harness.app().0), "project-document", "the text takes the keyboard, like a document");
}
...
fn a_page_is_drawn_as_a_picture_and_the_pages_are_walked_through() {
    ...
    assert_eq!(words[7..], ["sh".to_owned(), "1".to_owned(), format!("{PROJECT_DIR}/{PDF}")]);
    ...
    assert!(harness.is_focused("project-document"));
}
...
fn a_pdf_tab_is_kept_in_the_session_and_its_file_looked_for_when_it_comes_back() {
    ...
    assert_eq!(session.projects[0].tabs[0].kind, SessionTabKind::Pdf(PDF.to_owned()));
    ...
    let mut escaping = one_project(&scratch);
    escaping.restore_tabs(0, &record);
    assert!(kinds(&escaping).is_empty(), "a session file cannot name a PDF outside the project");

    // Gone from the disk, it says so rather than asking the container.
    fs::remove_file(scratch.paths().project.join(PDF)).expect("the PDF goes");
    let mut gone = one_project(&scratch);
    gone.restore_tabs(0, &read.value.projects[0]);
    let harness = harness(gone, SIZE.0, SIZE.1);
    assert_eq!(state(&harness.app().0, 0, 0), TabState::Missing);
    assert!(harness.screen().contains("tide tables.pdf is not in the project any more"), "{}", harness.screen());
}
...
/// A project folder with a Word document, an OpenDocument text and the office files that are
/// not opened.
fn with_office(name: &str) -> Scratch {
    let scratch = Scratch::new(name);
    let letters = scratch.paths().project.join("letters");
...
#[test]
fn a_word_document_opens_as_its_text_taken_out_in_the_projects_own_container() {
    ...
    assert_eq!(
        docx,
        ["exec", "qcode-firefly-base", "docx2txt", &format!("{PROJECT_DIR}/letters/to the harbour master.docx"), "-"]
    );
    let odt = spelled(&screen.read_command(key(&screen, 1)).expect("a document's text is read"));
    assert_eq!(odt, ["exec", "qcode-firefly-base", "odt2txt", &format!("{PROJECT_DIR}/letters/Tide.ODT")]);
    ...
    assert_eq!(entry(&screen), "project-document");
    let mut shown = harness(screen, SIZE.0, SIZE.1);
    let text = shown.screen();
...
fn spreadsheets_and_slides_are_still_not_opened() {
    ...
    let mut shown = harness(one_project(&scratch), SIZE.0, SIZE.1);
...
fn an_office_tab_comes_back_from_the_session() {
    ...
    apply(&mut screen, Msg::OpenFile("letters/Tide.ODT".to_owned()));
    let session = screen.session();
    assert_eq!(session.projects[0].tabs[0].kind, SessionTabKind::Office("letters/Tide.ODT".to_owned()));
    let read = Session::parse("session.toml", &session.to_toml());
    assert!(read.is_clean(), "{:?}", read.diagnostics);
    let mut again = one_project(&scratch);
    again.restore_tabs(0, &read.value.projects[0]);
