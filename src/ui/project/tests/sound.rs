...
use crate::ui::project::{Taken, entry};
use crate::workspace::{Session, SessionTabKind};
...
const DETAILS: &str = "Input File     : '/work/Project/sea sounds/fog horn.mp3'\nChannels       : 2\n\
                       Sample Rate    : 44100\nDuration       : 00:00:03.00 = 132300 samples\n";

/// A project folder with a sound in it, and a runtime folder beside it with a sound server's
...
impl Setting {
    fn new(name: &str) -> Self {
        ...
        let sounds = scratch.paths().project.join("sea sounds");
    ...
    /// The screen of `one_project`, looking for the sound server in `runtime`.
    fn screen(&self, runtime: Option<&Path>) -> ProjectScreen {
        one_project(&self.scratch).hearing_in(runtime.map(Path::to_path_buf))
    }
}
...
fn a_sound_plays_in_a_container_of_its_own_that_reaches_nothing_but_the_sound_server() {
    ...
    let project = setting.scratch.paths().project.display().to_string();
    assert_eq!(
        words(&screen, key),
        [
            "run",
            ...
            "--network=none",
            "--volume",
            &format!("{project}:/work/Project:ro,z"),
            "--workdir",
            "/work/Project",
            "qcode/base",
            "play",
            &format!("{PROJECT_DIR}/{SONG}"),
        ],
        "a one-off container, removed when it ends, with no network, the project read-only, the \
...
fn a_sound_that_does_not_play_shows_its_details_and_says_why() {
    ...
    for (why, words) in [
        ...
        assert_eq!(read, ["exec", "qcode-firefly-base", "soxi", &format!("{PROJECT_DIR}/{SONG}")]);
        apply(&mut screen, Msg::Described(key, 0, why, Ok(Taken::new(DETAILS.to_owned()))));
        assert_eq!(state(&screen, 0, 0), TabState::Running);
        assert_eq!(entry(&screen), "project-document", "the details take the keyboard, like a document");
        let mut shown = harness(screen, SIZE.0, SIZE.1);
...
fn a_playing_sound_says_how_to_stop_it_and_offers_to_play_again_once_it_ended() {
    ...
    assert_eq!(
        super::super::sound::playing(
            screen.project().expect("a project"),
            &screen.project().expect("a project").tabs().iter().collect::<Vec<_>>()
        ),
        ["qcode-firefly.play-0"],
    ...
    assert!(
        super::super::sound::playing(
            shown.app().0.project().expect("a project"),
            &shown.app().0.project().expect("a project").tabs().iter().collect::<Vec<_>>()
        )
        .is_empty(),
...
fn a_sound_brought_back_from_the_last_session_waits_to_be_asked() {
    ...
    assert_eq!(session.projects[0].tabs[0].kind, SessionTabKind::Sound(SONG.to_owned()));
...
fn a_sound_whose_file_is_gone_says_so_and_plays_nothing() {
    ...
    fs::remove_file(setting.scratch.paths().project.join(SONG)).expect("the sound goes");
    let key = key(&shown.app().0, 0);
    shown.send(Msg::Restart(key)).render();
    assert_eq!(state(&shown.app().0, 0, 0), TabState::Missing);
    assert!(shown.screen().contains("fog horn.mp3 is not in the project any more"), "{}", shown.screen());
}
