//! Following the project folder while other programs change it: a harness in a container writes
//! files through the project mount, and the tree shows them without being asked.
//!
...
//! and only in the project that is open; a change rereads the one folder it happened in.
//!
...
use super::files;
use super::{Msg, OpenProject};

/// Whether a project's folders are watched.
#[derive(Debug, Default)]
pub(super) enum Live {
    /// Not watched: the screen does not follow the disk, or the project is not the open one.
    #[default]
    Off,
...
/// A watch on the folders a project's tree shows.
#[derive(Debug)]
...
/// Keeps the watch of `project` on exactly the folders its tree shows: a watch is made the first
...
pub(super) fn follow(project: &mut OpenProject, run: &mut u64) -> Command<Msg> {
    let mut commands = Vec::new();
    if matches!(project.live, Live::Off) {
        match FolderWatch::new() {
            Ok(watch) => {
                *run += 1;
                commands.push(wait(project.id().to_owned(), *run, watch.changes()));
                project.live = Live::On(Watching { watch, run: *run, folders: BTreeMap::new() });
            }
            Err(_) => project.live = Live::Unavailable,
        }
    }
    let Live::On(watching) = &mut project.live else { return Command::batch(commands) };
    let visible = project.files.visible_folders();
    ...
    for key in visible {
        ...
        if watched && project.files.children(&key).is_some() && !project.files.is_loading(&key) {
            again.push(key.clone());
        }
        watching.folders.insert(key, watched.then_some(path));
    }
    commands.push(files::reread(project, again));
    Command::batch(commands)
}

/// Lets the watch of `project` go, which also ends its waiting thread.
pub(super) fn stop(project: &mut OpenProject) {
    if matches!(project.live, Live::On(_)) {
        project.live = Live::Off;
    }
}
...
pub(super) fn changed(project: &mut OpenProject, run: u64, batch: Vec<FolderChange>) -> Command<Msg> {
    let Live::On(watching) = &mut project.live else { return Command::none() };
    ...
    let next = wait(project.id.as_str().to_owned(), run, watching.watch.changes());
    let mut again = Vec::new();
    ...
    let reread = if everything {
        ...
        again.sort();
        again.dedup();
        files::reread(project, again)
    };
    Command::batch([reread, next])
}

/// Waits on a background thread for the next batch of the watch `run` of the project `id`.
