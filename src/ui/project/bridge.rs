//! The bridge between tabs on the project screen: each open project listens on its socket, the
...
use super::{Msg, OpenProject, ProjectScreen, Tab, TabKey, TabKind};
...
/// Whether a project listens on its socket.
#[derive(Debug, Default)]
pub(super) enum Link {
    /// Not yet: the screen does not bridge, or has not come round to this project.
    #[default]
    Off,
...
/// Opens the socket of every open project that has none yet, when the screen bridges, and waits
/// for its first call. A project whose socket cannot be opened says why once and is not tried
/// again while it stays open.
pub(super) fn follow(screen: &mut ProjectScreen) -> Command<Msg> {
    if !screen.bridging {
        return Command::none();
    }
    let mut commands = Vec::new();
    for project in &mut screen.projects {
        if !matches!(project.link, Link::Off) {
            continue;
        }
        match Listener::open(&project.paths.mcp()) {
            Ok(listener) => {
                screen.last_link += 1;
                commands.push(wait(project.id.as_str().to_owned(), screen.last_link, listener.inbox()));
                project.link = Link::On { listener, run: screen.last_link };
            }
            Err(error) => {
                project.link = Link::Failed;
                let toast =
                    Toast::warning(t!("bridge.closed", project = project.name.as_str())).body(error.to_string());
...
/// Waits on a background thread for the next call to the project `id`'s listener `run`.
fn wait(id: String, run: u64, inbox: Inbox) -> Command<Msg> {
    Command::perform(move || Msg::Bridge(id, run, inbox.next()))
}

/// Takes a call to the project `id`'s listener `run`, answers it, and waits for the next one.
/// `None` is a listener that was closed, which ends its waiting.
pub(super) fn called(screen: &mut ProjectScreen, id: &str, run: u64, call: Option<Call>) -> Command<Msg> {
    let Some(call) = call else { return Command::none() };
    let Some(project) = screen.projects.iter().find(|project| project.id.as_str() == id) else {
        return Command::none();
    };
    let Link::On { listener, run: current } = &project.link else { return Command::none() };
    if *current != run {
        return Command::none();
    }
...
/// Answers `call`, made to the project `id` at `now`.
pub(super) fn answer(screen: &mut ProjectScreen, id: &str, call: Call, now: Instant) -> Command<Msg> {
    ...
    let Some(sender) = sender(project, &question) else {
        call.answer(Answer::refused(t!("bridge.answer.stranger")));
        return Command::none();
    };
    match question.request {
        Request::List => {
            call.answer(list(project, sender));
            Command::none()
        }
...
/// The harness tab of `project` whose token the question carries.
fn sender(project: &OpenProject, question: &Question) -> Option<TabKey> {
    project
        .tabs
        .iter()
...
/// The profile of the harness tab `key`, when it has one the workspace still has.
fn profile_of(project: &OpenProject, key: TabKey) -> Option<&Profile> {
    let tab = project.tabs.iter().find(|tab| tab.key() == key)?;
    let TabKind::Profile(name) = tab.kind() else { return None };
    project.profiles.iter().find(|profile| profile.name.as_str() == name)
}

/// How a tab is named to the person and to the agents: its harness and its title.
fn named(project: &OpenProject, key: TabKey) -> String {
    let title = project.tabs.iter().position(|tab| tab.key() == key).map(|index| project.tab_label(index));
    let harness = profile_of(project, key).map(|profile| profile.harness.record().display_name);
    match (harness, title) {
...
/// The other harness tabs of `project`, the ones `sender` can send to.
fn others(project: &OpenProject, sender: TabKey) -> Vec<(usize, &Tab, &Profile)> {
    project
        .tabs
        .iter()
        .enumerate()
        .filter(|(_, tab)| tab.key() != sender)
        .filter_map(|(index, tab)| profile_of(project, tab.key()).map(|profile| (index, tab, profile)))
        .collect()
}

/// The answer to a list: every other harness tab of the project.
fn list(project: &OpenProject, sender: TabKey) -> Answer {
    let tabs: Vec<Listed> = others(project, sender)
        .into_iter()
        .map(|(index, tab, profile)| Listed {
            tab: tab.key().0.to_string(),
            title: project.tab_label(index),
...
/// The tab `wanted` names among the other harness tabs of `project`: by its id, or by its title
/// when exactly one tab has it.
fn target(project: &OpenProject, sender: TabKey, wanted: &str) -> Option<TabKey> {
    let others = others(project, sender);
    ...
    let titled: Vec<TabKey> = others
        .iter()
        .filter(|(index, _, _)| project.tab_label(*index) == wanted)
        .map(|(_, tab, _)| tab.key())
        .collect();
...
/// Handles a message from `from` to the tab `wanted` of the project `id`.
fn send(
    ...
    let Some(to) = target(project, from, wanted) else {
    ...
    let (Some(from_profile), Some(to_profile)) = (profile_of(project, from), profile_of(project, to)) else {
        call.answer(Answer::refused(t!("bridge.answer.no-tab", tab = wanted)));
        return Command::none();
    };
    let ends = (
    ...
    let (sender_name, receiver_name) = (named(project, from), named(project, to));
...
pub(super) fn allowed(screen: &mut ProjectScreen, from: TabKey, to: TabKey, allow: bool) {
    ...
    let Some(project) = screen.owner(to) else {
...
pub(super) fn still_asking(screen: &mut ProjectScreen, from: TabKey, to: TabKey) {
...
fn queue(screen: &mut ProjectScreen, to: TabKey, from: String, text: String, hop: u32, now: Instant) {
    screen.rules.received(to.0, hop, now);
    if let Some((_, tab)) = screen.owner_mut(to).and_then(|project| project.find(to)) {
...
pub(super) fn closed(screen: &mut ProjectScreen, keys: &[TabKey]) {
    for key in keys {
...
pub(super) fn letters(screen: &mut ProjectScreen, key: TabKey, action: Letters) {
...
pub(super) fn view(tab: &Tab, ui: &mut View<'_, Msg>) {
    ...
    ui.column(|ui| {
        ...
        ui.row(|ui| {
            ...
            ui.add(Button::new(label).on_press(Msg::Letters(key, Letters::Show(open)))).id("project-letters-show");
            ui.add(Button::new(t!("bridge.letters.discard")).on_press(Msg::Letters(key, Letters::Discard)))
                .id("project-letters-discard");
        })
        .gap(2)
        .padding(Padding { left: 2, right: 1, ..Padding::default() })
        .fill_width()
        .id("project-waiting-letters");
    })
    .gap(1)
    .fill_width();
}
