...
fn shelf_key(profile: &str) -> HistoryKey {
    HistoryKey { workspace: "firefly".to_owned(), profile: profile.to_owned() }
}

/// The number of the reading the screen last started for `profile` of the open workspace.
fn generation(screen: &WorkspaceScreen, profile: &str) -> u64 {
    screen
        .workspace()
...
/// A screen of `one_workspace` with a blank tab open and its page shown.
...
fn choosing_a_conversation_resumes_it_inside_the_container() {
    ...
    let screen = &harness.app().0;
    let chosen = &screen.workspace().expect("a workspace").tabs()[0];
    ...
    assert_eq!(recorded.workspaces[0].tabs[0].conversation.as_deref(), Some(id), "the session keeps it");
}

#[test]
fn a_codex_conversation_is_resumed_with_its_subcommand() {
    let scratch = Scratch::new("history-codex");
    let workspaces = vec![workspace("firefly", "Firefly", scratch.paths(), vec![profile("codex", HarnessKind::Codex)])];
...
fn restore(session: &Session, scratch: &Scratch) -> WorkspaceScreen {
    let mut screen = one_workspace(scratch);
...
fn conversation_of(screen: &WorkspaceScreen, index: usize) -> Option<String> {
    screen.workspace().expect("a workspace").tabs()[index].conversation().map(str::to_owned)
}
...
fn a_restored_new_chat_resumes_the_newest_conversation_it_could_have_had() {
    ...
    assert!(screen.workspace().expect("a workspace").tabs()[0].state().is_starting(), "the open tab woke");
    let found = vec![
    ...
    let recorded = screen.session();
    let kept: Vec<Option<&str>> = recorded.workspaces[0].tabs.iter().map(|tab| tab.conversation.as_deref()).collect();
...
fn the_history_has_no_brackets_in_any_glyph_mode_and_reads_as_turkish() {
    ...
    let screen = WorkspaceScreen::new(Some(engine()), HostUser::ImageDefault, vec![open]);
    let mut harness = harness(screen, SIZE.0, SIZE.1);
    harness.send(Msg::NewTab);
