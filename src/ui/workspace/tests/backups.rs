//! Backing the open workspaces up: when the screen asks for a backup, what it says about the last
...
use crate::ui::workspace::{BackupOf, BackupTrouble, Part, backups, leaving};
...
/// A harness of `one_workspace` with motion off, so a toast is on screen as soon as it is sent.
fn backing_up(scratch: &Scratch, every: BackupEvery) -> Harness<Screen> {
    let mut harness = harness(one_workspace(scratch).backing_up(every), SIZE.0, SIZE.1);
...
fn backed(made: Result<Snapshot, BackupTrouble>) -> Msg {
    Msg::BackedUp { workspace: "firefly".to_owned(), name: "Firefly".to_owned(), made, missed: Vec::new() }
}
...
#[test]
fn an_open_workspace_is_backed_up_one_interval_after_it_opens_and_not_at_once() {
    let scratch = Scratch::new("backup-timer");
    let mut harness = backing_up(&scratch, BackupEvery::Fifteen);
    assert!(!harness.screen().contains(FAILED), "nothing is backed up as the workspace opens");
...
#[test]
fn a_changed_interval_replaces_the_timers_of_the_open_workspaces() {
...
fn without_an_engine_nothing_is_backed_up_and_nothing_is_said() {
    ...
    let screen = WorkspaceScreen::new(None, HostUser::ImageDefault, workspaces).backing_up(BackupEvery::Five);
...
#[test]
fn a_workspace_closed_from_the_rail_is_backed_up_as_it_goes() {
    let scratch = Scratch::new("backup-close");
    let mut harness = backing_up(&scratch, BackupEvery::Fifteen);
    harness.send(Msg::CloseWorkspace(0)).advance(Duration::from_millis(10));
    assert!(harness.app().0.workspace().is_none(), "the workspace left the rail");
...
/// The row of the panel's workspace widget that reads one of `labels`, read with only that widget
...
/// The row of the workspace widget that names the last backup.
fn backup_row(harness: &mut Harness<Screen>) -> String {
    info_row(harness, &["Last backup", "Son yedek"])
}

/// The row of the workspace widget that names what the backup leaves out.
...
fn the_panel_says_when_the_last_backup_was_made() {
    ...
    let mut harness = backing_up(&scratch, BackupEvery::Fifteen);
    assert!(backup_row(&mut harness).contains("not yet"), "a workspace never backed up");
...
fn without_an_engine_the_panel_says_why_there_is_no_backup() {
    ...
    let workspaces = vec![workspace("firefly", "Firefly", scratch.paths(), Vec::new())];
    let mut harness = harness(WorkspaceScreen::new(None, HostUser::ImageDefault, workspaces), SIZE.0, SIZE.1);
...
#[test]
fn what_is_left_to_back_up_as_qcode_closes_is_every_workspace_of_the_rail() {
    ...
    let workspaces = vec![workspace("firefly", "Firefly", scratch.paths(), Vec::new())];
    assert_eq!(leaving(&WorkspaceScreen::new(None, HostUser::ImageDefault, workspaces)), None, "nor does no engine");
    assert_eq!(leaving(&WorkspaceScreen::new(Some(engine()), HostUser::ImageDefault, Vec::new())), None);
}

/// A moment for a menu to settle.
const MOMENT: Duration = Duration::from_millis(400);

/// `one_workspace` with its `workspace.qcode` on disk, the way the store leaves a workspace.
fn with_file(name: &str) -> (Scratch, Harness<Screen>) {
    let scratch = Scratch::new(name);
    fs::write(scratch.paths().file, file("firefly", "Firefly", &["claude-sub"]).to_toml()).expect("a workspace file");
...
/// What the open workspace's backup leaves out.
fn skipped(harness: &Harness<Screen>) -> Vec<String> {
    harness.app().0.workspace().expect("a workspace is open").backup_skip().to_vec()
}

/// What the workspace's file on disk leaves out.
fn on_disk(scratch: &Scratch) -> Vec<String> {
    let text = fs::read_to_string(scratch.paths().file).expect("the workspace file");
    WorkspaceFile::parse("workspace.qcode", &text).value.expect("a workspace").backup_skip
}
...
fn a_folder_is_left_out_from_its_menu_and_taken_in_again_the_same_way() {
    ...
    assert_eq!(on_disk(&scratch), ["src"], "the workspace file keeps it");
    select(&mut harness, "README.md");
    assert_ne!(icon_colour(&harness, "src"), plain, "the row looks left out");
    assert!(left_out_row(&mut harness).contains("src"), "and the workspace widget names it");
...
#[test]
fn a_workspace_file_that_cannot_be_written_says_so_and_nothing_changes() {
...
fn what_the_file_leaves_out_is_what_the_workspace_opens_with_and_it_speaks_turkish() {
    ...
    let workspaces = vec![OpenWorkspace::new(&carried, scratch.paths(), Vec::new())];
...
#[test]
fn the_workspace_widget_opens_the_list_of_backups_and_a_workspace_never_backed_up_has_none() {
...
fn each_backup_says_when_it_was_made_and_how_many_files_it_changed() {
    ...
    assert!(harness.is_focused("workspace-backups"), "the list has the keyboard");
    let text = harness.screen();
...
fn a_backup_brought_back_closes_the_list_and_says_so() {
    ...
    let text = harness.screen();
    assert!(text.contains("Another QCode is backing this workspace up"), "{text}");
...
/// The profiles whose conversations the next round of the open workspace takes.
fn used(screen: &WorkspaceScreen) -> Vec<String> {
    backups::used(screen.workspace().expect("a workspace is open")).into_iter().collect()
}

/// A screen of `Firefly` carrying a Claude Code and an opencode profile, reaching `engine`.
fn two_profiles(scratch: &Scratch, engine: Engine) -> WorkspaceScreen {
    let profiles = vec![profile("claude-sub", HarnessKind::ClaudeCode), profile("oc", HarnessKind::OpenCode)];
    let workspaces = vec![workspace("firefly", "Firefly", scratch.paths(), profiles)];
...
fn a_round_takes_the_conversations_of_every_profile_whose_tab_was_open_since_the_last_one() {
    ...
    assert!(screen.workspace().is_none());
...
#[test]
fn conversations_that_could_not_be_backed_up_are_said_once_like_the_workspace() {
    ...
    assert!(!harness.screen().contains(FAILED), "nor is the workspace's own failure after it:\n{}", harness.screen());
...
/// The list of backups of `with_file`'s workspace, made the list of `claude-sub`'s conversations
...
fn the_list_of_backups_switches_to_a_profiles_conversations_and_back() {
    ...
    let first = reading(&harness);
    let text = harness.screen();
    assert!(text.contains("Work files"), "the choice shows what the list is of:\n{text}");
    harness.send(Msg::BackupsOf(1));
    assert_ne!(reading(&harness), first, "read afresh");
    harness.send(Msg::BackupsRead(first, Ok(two())));
    assert!(!harness.screen().contains("09:05"), "the answer for the workspace's list is dropped");
    let text = harness.screen();
    ...
    harness.send(Msg::BackupsOf(0));
    assert!(harness.screen().contains("Work files"), "{}", harness.screen());
    harness.set_locale("tr").render();
    harness.send(Msg::BackupsOf(1));
...
#[test]
fn a_files_list_and_a_workspace_with_no_profile_offer_no_choice() {
    ...
    assert!(!file_list.screen().contains("Work files"), "{}", file_list.screen());
    ...
    let workspaces = vec![workspace("firefly", "Firefly", scratch.paths(), Vec::new())];
    let mut harness = harness(WorkspaceScreen::new(Some(engine()), HostUser::ImageDefault, workspaces), SIZE.0, SIZE.1);
    harness.send(Msg::ShowBackups(None));
    assert!(!harness.screen().contains("Work files"), "{}", harness.screen());
}
...
fn conversations_brought_back_close_the_list_and_a_missing_home_is_said_plainly() {
    ...
    let text = harness.screen();
    assert!(text.contains("claude-sub has no home in this workspace"), "{text}");
    ...
    assert!(!text.contains("09:05"), "the list closed:\n{text}");
    assert!(backup_row(&mut harness).contains("not yet"), "the workspace's own last backup is not the conversations'");
}

/// Unfolds only the panel's workspace widget, where the backup's switch and rows are.
...
/// Whether the workspace's file on disk backs its assets up.
fn assets_on_disk(scratch: &Scratch) -> bool {
    let text = fs::read_to_string(scratch.paths().file).expect("the workspace file");
    WorkspaceFile::parse("workspace.qcode", &text).value.expect("a workspace").backup_assets
}

#[test]
fn the_assets_switch_is_written_into_the_workspace_file_and_back_out() {
    ...
    assert!(harness.app().0.workspace().expect("open").backs_up_assets());
...
#[test]
fn a_round_takes_the_assets_when_the_workspace_asks_for_them() {
    let (_scratch, mut harness) = with_file("assets-round");
    harness.send(Msg::BackupAssets(true));
    let left = leaving(&harness.app().0).expect("backups are on");
    let lines = crate::ui::settings::testing::translated("en", || left.back_up());
    assert_eq!(lines.len(), 2, "the workspace and its assets each failed without an engine: {lines:?}");
    ...
    let left = leaving(&harness.app().0).expect("backups are on");
    let lines = crate::ui::settings::testing::translated("en", || left.back_up());
    assert_eq!(lines.len(), 1, "off, only the workspace is backed up: {lines:?}");
}
...
/// The row of the workspace widget that says how much the backup holds.
...
fn the_panel_says_how_much_the_backup_holds_and_reads_it_again_after_a_round() {
    ...
    let git = scratch.paths().backup().join("Code.git");
    ...
    let workspaces = vec![workspace("firefly", "Firefly", scratch.paths(), Vec::new())];
    let mut offline = super::harness(WorkspaceScreen::new(None, HostUser::ImageDefault, workspaces), SIZE.0, SIZE.1);
