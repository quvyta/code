...
#[test]
fn a_new_folder_is_made_at_the_root_from_the_workspace_folders_row() {
    ...
    assert!(line.contains("Firefly"), "the workspace folder is the top row, by the workspace's name:\n{line}");
...
fn names_are_checked_as_they_are_typed() {
    ...
    assert!(fs::read_dir(workspace_dir(&scratch)).map(Iterator::count).unwrap_or_default() == 2, "nothing was made");
}
...
#[test]
fn a_link_out_of_the_workspace_is_an_entry_and_deleting_it_leaves_its_target() {
...
fn the_keyboard_reaches_the_same_menus() {
    ...
    assert!(harness.is_focused("workspace-files"), "{}", harness.screen());
...
#[test]
fn the_keyboard_reaches_the_workspace_folders_menu_from_its_row() {
    let scratch = Scratch::new("root-keys");
    let mut harness = files_harness(&scratch);
    harness.click_text("src").advance(MOMENT);
    harness.press("home").advance(MOMENT);
    assert_eq!(tree(&harness).selected(), Some(""), "the top row is the workspace folder");
    harness.press("shift+f10").advance(MOMENT);
    let text = harness.screen();
...
#[test]
fn an_empty_workspace_folder_still_has_its_row_to_make_the_first_entry() {
    let scratch = Scratch::new("empty");
    let root = workspace_dir(&scratch);
    fs::remove_dir_all(&root).expect("clear the workspace");
    fs::create_dir_all(&root).expect("an empty workspace folder");
    let mut harness = files_harness(&scratch);
    let text = harness.screen();
...
fn refresh_and_coming_back_read_the_open_folders_again() {
    ...
    fs::remove_file(workspace_dir(&scratch).join("GUIDE.md")).expect("gone again");
...
/// A workspace folder with a picture, a Markdown document and a text file with an awkward name.
fn with_files(name: &str) -> Scratch {
    let scratch = Scratch::new(name);
    let workspace = scratch.paths().code;
    fs::create_dir_all(workspace.join("art")).expect("a folder for pictures");
    fs::create_dir_all(workspace.join("guide")).expect("a folder for documents");
    fs::write(workspace.join("art").join("the logo.png"), [0x89, b'P', b'N', b'G']).expect("a picture");
    fs::write(workspace.join("guide").join("harbour.md"), "# Harbour notes\n\nThe tide turns at *six*.\n")
        .expect("a document");
    fs::write(workspace.join("it's $HOME.txt"), "text\n").expect("a text file");
    fs::write(workspace.join("voice.m4a"), [0, 1, 2]).expect("a sound nothing built in plays");
    scratch
}
...
/// The labels of the open workspace's tabs, as the strip shows them.
fn labels(harness: &Harness<Screen>) -> Vec<TabKind> {
    kinds(&harness.app().0)
}

#[test]
fn a_text_file_opens_in_nano_in_the_workspaces_own_container_as_one_word() {
    ...
    assert_eq!(
        args,
        ["exec", "--interactive", "--tty", "qcode-firefly-base", "nano", &format!("{CODE_DIR}/it's $HOME.txt")],
...
fn the_chosen_editor_is_the_one_a_file_opens_in() {
    ...
    let args = words(&screen, key(&screen, 0));
    assert_eq!(args[args.len() - 2..], ["vim".to_owned(), format!("{CODE_DIR}/src/main.rs")]);
...
#[test]
fn a_picture_is_drawn_by_chafa_in_the_workspaces_own_container() {
    ...
    assert_eq!(args.last().map(String::as_str), Some(format!("{CODE_DIR}/art/the logo.png").as_str()));
...
fn a_drawn_picture_stays_with_a_quiet_way_to_draw_it_again_and_no_ending_line() {
    ...
    let tab = &app.workspace().expect("a workspace").tabs()[0];
...
fn a_markdown_document_is_shown_by_qcode_and_edited_in_a_tab_of_its_own() {
    ...
    assert!(harness.is_focused("workspace-document"), "the document takes the keyboard");
    ...
    assert_eq!(args[args.len() - 2..], ["nano".to_owned(), format!("{CODE_DIR}/guide/harbour.md")]);
}
...
fn a_document_shown_again_is_read_again() {
    ...
    fs::write(scratch.paths().code.join("guide").join("harbour.md"), "# Changed in the editor\n").expect("a change");
...
fn a_file_opened_twice_is_one_tab() {
    ...
    assert_eq!(screen.workspace().and_then(OpenWorkspace::active_tab).map(Tab::key), Some(key(&screen, 0)));
}
...
#[test]
fn a_path_that_climbs_out_of_the_workspace_is_refused_everywhere() {
...
#[cfg(unix)]
#[test]
fn a_document_that_links_out_of_the_workspace_is_not_read() {
    let scratch = with_files("link");
    let outside = scratch.0.join("outside.md");
    fs::write(&outside, "# Not the workspace's\n").expect("a file beside the workspace");
    std::os::unix::fs::symlink(&outside, scratch.paths().code.join("link.md")).expect("a link");
    let mut harness = harness(one_workspace(&scratch), SIZE.0, SIZE.1);
    harness.send(Msg::OpenFile("link.md".to_owned())).render();
    let text = harness.screen();
    assert!(!text.contains("Not the workspace's"), "{text}");
    assert!(text.contains("leads outside the workspace folder"), "{text}");
}
...
fn file_tabs_are_kept_in_the_session_and_come_back_one_at_a_time() {
    ...
    let saved: Vec<SessionTabKind> = session.workspaces[0].tabs.iter().map(|tab| tab.kind.clone()).collect();
    assert_eq!(
        saved,
        [
...
fn a_restored_tab_whose_file_is_gone_says_so() {
    ...
    let text = harness.screen();
    assert!(text.contains("guide/gone.md is not in the workspace any more"), "{text}");
    ...
    assert!(harness.screen().contains("gone.txt is not in the workspace any more"), "{}", harness.screen());

    // The file comes back and looking again finds it.
    fs::write(scratch.paths().code.join("guide").join("gone.md"), "# Back again\n").expect("the file returns");
...
fn file_tabs_draw_nothing_bracketed_nothing_but_ascii_in_ascii_mode_and_turkish_in_turkish() {
    ...
    let mut document = harness(one_workspace(&scratch), SIZE.0, SIZE.1);
