...
//!
//! Under each profile's new chat come its newest conversations in the workspace, read out of the
...
enum Row {
    ...
    Gap,
    /// The heading of the section of the profile at this position of the workspace's profiles.
    Profile(usize),
    ...
    /// Says the profile has had no conversation in the workspace yet.
    NoChats,
...
/// The rows of the page for `workspace`: the shell first, then a section for every profile in the
/// order the workspace offers them, or the way to a first profile when there is none.
///
/// Without an engine nothing was read, so a section is only its new chat.
fn rows(screen: &WorkspaceScreen, workspace: &OpenWorkspace) -> Vec<Row> {
    let mut rows = vec![Row::Shell];
    if workspace.profiles().is_empty() {
        rows.extend([Row::Gap, Row::MakeProfile]);
    }
    for (index, profile) in workspace.profiles().iter().enumerate() {
        ...
        let Some(shelf) = workspace.history.get(name) else { continue };
        match shelf.shown() {
            Shown::Nothing => {}
...
pub(super) fn keep_row(screen: &mut WorkspaceScreen, change: impl FnOnce(&mut WorkspaceScreen)) {
    let page = |screen: &WorkspaceScreen| {
        screen
            .workspace()
            .filter(|workspace| workspace.active_tab().is_some_and(|tab| tab.kind() == &TabKind::New))
            .map(|workspace| rows(screen, workspace))
    };
...
fn item(workspace: &OpenWorkspace, row: &Row, look: &Look, ready: bool) -> ListItem {
    let dot = look.dot.as_str();
    match row {
        Row::Shell => ListItem::new(t!("workspace.tab.shell"))
            .icon("prompt", None)
            .detail(t!("workspace.choose.shell-detail"))
            .faint(!ready),
        Row::Gap => ListItem::gap(),
        Row::Profile(index) => {
            let Some(profile) = workspace.profiles().get(*index) else { return ListItem::gap() };
            let name = profile.name.as_str();
            let harness = profile.harness.record().display_name;
            // A profile the workspace does not carry yet is offered all the same; its heading says
            // that choosing from it adds it, so the file changing is no surprise.
            ListItem::header(if workspace.carries(name) {
                t!("workspace.choose.section", profile = name, harness = harness, dot = dot)
            } else {
                t!("workspace.choose.section-adds", profile = name, harness = harness, dot = dot)
            })
        }
        Row::NewChat(_) => ListItem::new(t!("workspace.choose.new-chat")).icon("add", None).faint(!ready),
        Row::Window(_) => ListItem::new(t!("workspace.choose.window"))
            .icon("window-maximize", None)
            .detail(t!("workspace.choose.window-detail"))
            .faint(!ready),
        Row::Chat(_, conversation) => {
            let title = conversation.title.clone().unwrap_or_else(|| t!("workspace.history.untitled"));
            ListItem::new(format!("{}{title}", look.indent))
                .detail(history::when(conversation.used_ms, look.now))
                .faint(!ready)
        }
        Row::ShowAll(_, more) => {
            ListItem::new(format!("{}{}", look.indent, t!("workspace.history.show-all", more = *more, dot = dot)))
                .faint(!ready)
        }
        Row::NoChats => ListItem::new(format!("{}{}", look.indent, t!("workspace.history.none"))).faint(true),
        Row::Reading => ListItem::new(format!("{}{}", look.indent, t!("workspace.history.reading"))).faint(true),
        Row::Unread(reason) => ListItem::new(format!("{}{}", look.indent, t!("workspace.history.unread")))
            .detail(reason.clone())
            .faint(true),
        Row::MakeProfile => ListItem::new(t!("workspace.make-profile"))
            .icon("add", None)
            .detail(t!("workspace.no-profiles"))
            .faint(!ready),
    }
}
