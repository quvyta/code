...
//! workspace folder, on the host, and never outside it.
//!
//! Every operation takes the workspace folder and tree keys, never a path, and turns keys into paths
...
impl NameProblem {
    ...
    pub fn message(&self) -> String {
        let key = match self {
            Self::Empty => "workspace.files.name-empty",
            Self::Slash => "workspace.files.name-slash",
            Self::Nul => "workspace.files.name-nul",
            Self::Dots => "workspace.files.name-dots",
            Self::Taken => "workspace.files.name-taken",
        };
        t!(key)
    }
}
...
pub enum FileError {
    ...
    Name(NameProblem),
    /// The entry would be reached, or land, outside the workspace folder.
    Outside,
...
impl FileError {
    ...
    pub fn message(&self) -> String {
        match self {
            ...
            Self::Outside => t!("workspace.files.outside"),
            Self::IntoItself => t!("workspace.files.into-itself"),
            Self::Taken(name) => t!("workspace.files.taken", name = name.as_str()),
            Self::CrossDevice => t!("workspace.files.cross-device"),
...
///
/// Every folder on the way must be a real folder of the workspace, not a link, so the path cannot
/// lead out of it.
...
/// The path of the folder `key`, which must be a real folder inside the workspace.
...
/// # Errors
///
/// A name that cannot be used, a folder outside the workspace, a name already there, or what the
/// system said.
...
/// # Errors
///
/// A name that cannot be used, an entry outside the workspace, a name already there, or what the
/// system said.
...
/// # Errors
///
/// An entry or folder outside the workspace, a folder into itself, a name already there, another
...
/// # Errors
///
/// An entry outside the workspace, or what the system said.
...
mod tests {
    ...
    #[test]
    fn keys_that_reach_outside_the_workspace_are_refused() {
    ...
    #[test]
    fn a_link_out_of_the_workspace_is_never_followed() {
